...
test.describe('Módulo Consultar Servicios', () => {
    ...
    await page.route('**/api/admin/services**', async (route) => {
      ...
      if (request.method() === 'GET') {
          ...
          await route.fulfill({
            ...
            body: JSON.stringify({
              ...
              data: [
                ...
                { id: 2, nombre: 'Reparación Básica', descripcion: 'Reparaciones simples', categoria: 'Reparación', precio: 80, activo: false, fechaCreacion: '2024-01-10' },
                { id: 3, nombre: 'Consultoría Técnica', descripcion: 'Asesoramiento profesional', categoria: 'Consultoría', precio: 200, activo: true, fechaCreacion: '2024-02-20' },
                { id: 4, nombre: 'Instalación Completa', descripcion: 'Instalación completa de sistemas', categoria: 'Instalación', precio: 300, activo: true, fechaCreacion: '2024-03-05' }
              ],
              total: 4
            })
          });
        ...
        if (serviceId === '1') {
          await route.fulfill({
            status: 403,
            body: JSON.stringify({ mensaje: 'No se puede eliminar este servicio.' })
          });
        } else {
          await route.fulfill({
            status: 200,
  ...
  test('1. Debería renderizar la tabla de servicios correctamente', async ({ page }) => {
    await page.goto('/admin/consultar-servicios');

    await expect(page.getByText('Consultar Servicios')).toBeVisible();
    ...
    await expect(page.getByRole('button', { name: /nuevo servicio|new service/i })).toBeVisible();
  });
  ...
  test('5. Debería desactivar un servicio activo', async ({ page }) => {
    await page.goto('/admin/consultar-servicios');
    ...
    // Hacer clic en el botón de desactivar del primer servicio activo
  ...
  test('6. Debería activar un servicio inactivo', async ({ page }) => {
    await page.goto('/admin/consultar-servicios');
    ...
    // Hacer clic en el botón de activar del servicio inactivo
  ...
  test('7. Debería eliminar un servicio con confirmación', async ({ page }) => {
    await page.goto('/admin/consultar-servicios');

    const deleteButtons = page.getByRole('button', { name: /eliminar|delete/i });
    await deleteButtons.nth(1).click(); // Eliminar el segundo servicio (no el protegido)

    await expect(page.getByText(/¿Está seguro de que desea eliminar este servicio?/i)).toBeVisible();
    
    const requestPromise = page.waitForRequest(req => 
      req.url().includes('/services/') && req.method() === 'DELETE'
    );
    ...
    await expect(page.getByText(/servicio.*eliminado|service.*deleted/i)).toBeVisible();
  });

  test('8. Debería impedir eliminar un servicio protegido', async ({ page }) => {
    await page.goto('/admin/consultar-servicios');

    const deleteButtons = page.getByRole('button', { name: /eliminar/i });
    await deleteButtons.first().click(); // Intentar eliminar el primer servicio (protegido)
  ...
  test('14. Debería navegar a detalles de servicio', async ({ page }) => {
    await page.goto('/admin/consultar-servicios');

    // Hacer clic en el nombre del primer servicio
    await page.getByText(/Mantenimiento Preventivo/i).click();

    // Verificar que se muestra una página o modal de detalles
    await expect(page.getByText(/detalles del servicio/i)).toBeVisible();
    await expect(page.getByText(/servicio mensual/i)).toBeVisible();
  ...
  test('15. Debería permitir edición rápida desde la lista', async ({ page }) => {
    ...
    await expect(page.getByText(/editar servicio|edit service/i)).toBeVisible();
  });
});