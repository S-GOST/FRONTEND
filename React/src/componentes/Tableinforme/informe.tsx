...
const TableInformes = () => {
  ...
  return (
    <div className="informes-page">
      ...
      {showModal && (
        <div className="modal-overlay" onClick={closeModal} role="button" tabIndex={0} onKeyDown={(e) => { if (e.target === e.currentTarget && (e.key === 'Enter' || e.key === ' ')) { closeModal(); } }}>
...
<textarea id="auto-id-36315" name="recomendaciones" value={formData.recomendaciones || ''} onChange={handleInputChange} rows={3} />
      ...
      {showReportModal && (
        <div className="modal-overlay" onClick={() => setShowReportModal(false)} role="button" tabIndex={0} onKeyDown={(e) => { if (e.target === e.currentTarget && (e.key === 'Enter' || e.key === ' ')) { setShowReportModal(false); } }}>
          <div className="modal-container" onClick={e => e.stopPropagation()} role="dialog" tabIndex={-1}>
            <div className="modal-header">
